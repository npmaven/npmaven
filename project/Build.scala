...
object RootBuild extends Build {
  ...
  // Copy the site generated by sbt-site's Pamflet to /webapp on the classpath for the jetty WebAppContext to find
  val generateSite = generateSiteTaskKey <<= (resourceManaged in Compile, siteDirectory) map { (rsrc: File, site: File) =>
    ...
    val target = rsrc / "webapp" / "site"
    IO.copyDirectory(site, target)
    relPaths.map(name => new File(target, name))
  }
